                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td className={`p-4 text-center font-bold ${isLow ? 'text-red-600 bg-red-50/30 rounded-lg' : 'text-gray-800'}`}>
                          <div className="tabular-nums font-mono">{item.stock.toFixed(2)}</div>
                          {/* Mini visual stock level gauge */}
                          <div className="w-20 mx-auto bg-gray-200 h-1.5 rounded-full overflow-hidden mt-1">
                            <div 
                              className={`h-full rounded-full transition-all ${isLow ? 'bg-red-500' : ratio < 50 ? 'bg-amber-500' : 'bg-emerald-500'}`} 
                              style={{ width: `${Math.max(ratio, 8)}%` }} 
                            />
                          </div>
                          {isLow && <span className="block text-[10px] text-red-500 font-semibold mt-0.5">{t('Low Stock')}</span>}
                        </td>
                        <td className="p-4 text-center text-gray-500 font-mono tabular-nums">{item.minStock.toFixed(2)}</td>
